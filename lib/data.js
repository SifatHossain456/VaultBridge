...
export function getBridgeFee(amount, token, priceOverride) {
  const num = parseFloat(amount)
  if (!num || isNaN(num)) return null
  const tok = TOKENS.find(t => t.symbol === token)
  if (!tok) return null
  const price    = priceOverride ?? tok.price
  const usdValue = num * price
  const feeUsd   = Math.max(0.5, usdValue * 0.0005)
  const isStable = token === 'USDC' || token === 'USDT'
  const feeToken = (feeUsd / price).toFixed(isStable ? 2 : 6)
  const receiveAmount = (num - parseFloat(feeToken)).toFixed(isStable ? 2 : 6)
  return {
    feeToken, feeUsd: feeUsd.toFixed(2),
    receiveAmount,
    receiveUsd: ((num - parseFloat(feeToken)) * price).toFixed(2),
    time: '~2 min',
  }
}
